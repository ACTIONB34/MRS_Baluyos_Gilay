		...
		if (reservation.getSelectedSeats().length > 0) {
			_addReservation({
				...
				ticketInfo: {
					...
					total: reservation.getSelectedSeats().length * reservation.schedule.price,
				},
				...movie,
			});
...
function populateViewSeats(seats, scheduleId) {
	...
	table +=
		"<tr>" +
			"<td></td>" +
			"<td><div class='d-flex p-2 bd-highlight bg-light text-dark justify-content-center align-text-top' style='height: 45px'>Door</div></td>" +
			"<td></td>" +
			"<td></td>" +
			"<td></td>" +
			"<td></td>" +
...
function updateTicketInfo(
	...
	$(selector).html(
		...
		"<strong>Tickets: </strong> " + noOfseats + "<br>" +
		"<strong>Seats selected: </strong>" + seats + "</p><hr>" +
		"<strong>Total:</strong> P " + 	total + "<br> <br>" +
		(noOfseats > 0 ?
